...
###<span style="color:darkblue">Simplifying the codes</span>
...
The reason we are simplifying the code is so that you won't get confused once we integrate the analyser node, data collection methods and HTML5 canvas element.

###<span style="color:darkblue">Using the AnalyserNode</span>

Now before we get to the exciting part, I would like to firstly introduce you to the [AnalyserNode](https://developer.mozilla.org/en-US/docs/Web/API/AnalyserNode) which is essential in helping us visualising our audio waveforms. 

**<span style="color:darkblue">So what is a AnalyserNode?</span>** 
...
Our audio source is basically our oscillator node, so what the analyser node does is to to extract frequency, waveform, and other data from that. 
In order to do that, you have to include the analyser node into your connections. However before that we firstly must create the analyser node using the [AudioContext.createAnalyser()](https://developer.mozilla.org/en-US/docs/Web/API/AudioContext/createAnalyser) method.
...
```
An interesting thing to take note is that you do not have to connect the analyser node to any output for it to work. It would work as long as its input is connect to the source directly or via another node. For now, we are connecting it to the `context.destination` so that we would be able to hear our sounds.
